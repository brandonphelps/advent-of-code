...
fn get_total_calibrations(input: &str) -> u32 {
    ...
    for (index, line) in input.split("\n").enumerate() {
        let cali_bration = get_calibration(line).expect(&format!("Failed to get calibration on line {:?}", index));
        cali_sum += cali_bration;
    }
    cali_sum
}

pub fn day_one() {
    let input_str = r#"hcpjssql4kjhbcqzkvr2fivebpllzqbkhg
...
ffnrprtnine1tjznmckv5sixczv"#;
    ...
    let cali_sum = get_total_calibrations(input_str);
    println!("Calibration sum: {:?}", cali_sum);
}
...
mod tests {
    ...
    #[case("1a2", 12)]
    #[case("ab4sjkl", 44)]
    #[case("hcpjssql4kjhbcqzkvr2fivebpllzqbkhg", 45)]
    #[case("ffnrprtnine1tjznmckv5sixczv", 96)]
    #[case("5z14", 54)]
    #[case("twone5four", 24)]
    #[case("bhkjl84fiveeight", 88)]
    #[case("bc2", 22)]
    #[case("khoneightsevenrflbbdmfourm6nhfdxzpkhlbsdqgvtctlp", 16)]
    #[case("nineseven2sixnineqd", 99)]
    #[case("ffnrprtnine1tjznmckv5sixczv", 96)]
    fn test_get_calibration(#[case] input: &str, #[case] expected: u32) {
        let cali = get_calibration(input).unwrap();
        assert_eq!(cali, expected);
    }
...
zoneight234
7pqrstsixteen"#;
        assert_eq!(get_total_calibrations(input), 281);
    }

    #[rstest]
    #[case("absjkl")]
    fn test_get_calibration_missing(#[case] input: &str) {
        assert!(matches!(
            get_calibration(input),
